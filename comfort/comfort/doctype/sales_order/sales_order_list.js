...
function add_not_in_po_check(list) {
	...
	var check = list.page.add_field({
		...
		async change() {
			...
			if (check.get_value() == 1) {
				await frappe.call({
					method: 'comfort.comfort.doctype.sales_order.sales_order.get_sales_orders_in_purchase_order',
					callback: r => {
						list.filter_area.filter_list.add_filter('Sales Order', 'name', 'not in', r.message, true);
					}
				});
			}
	...
	check.$input.unbind('input'); // To avoid dublicate calls
