...
from ikea_api import (Cart, OrderCapture, Purchases, get_authorized_token,
                      get_guest_token)
...
class PurchaseOrder(Document):
    ...
    def calculate_totals(self):  # TODO: Refactor
        self.total_weight, self.total_amount, \
            self.sales_order_cost, self.items_to_sell_cost = 0, 0, 0, 0
    ...
    def get_templated_items_for_api(self, split_combinations=False):
        ...
        if self.sales_orders and len(self.sales_orders) > 0:
            ...
            if split_combinations:
                ...
                so_items = [
                    d for d in so_items if d.item_code not in parent_items]
    ...
    def get_delivery_services(self):
        ...
        self.update({
            ...
            'cannot_add_items': as_json(delivery_services['cannot_add']) if delivery_services['cannot_add'] else None
        })

    def before_save(self):
        # TODO: Check if there any IkeaCarts that contains those Sales Orders
        sales_order_names = list(
            set([s.sales_order_name for s in self.sales_orders]))
    ...
    @frappe.whitelist()
    def before_submit_events(self, purchase_id, purchase_info_loaded, purchase_info, delivery_cost=None):
    ...
    def get_sales_order_items_for_bin(self):
        if self.sales_orders and len(self.sales_orders) > 0:
            ...
            so_items_map = {}
            for d in so_items:
                if d.item_code not in so_items_map:
                    so_items_map[d.item_code] = 0
                so_items_map[d.item_code] += d.qty

            return so_items_map.items()
...
class IkeaCartUtils():
    ...
    def get_token(self, authorize=False):
        ...
        if not authorize:
            if not self.guest_token or get_datetime(doc.guest_token_expiration_time) <= now_datetime():
            ...
            if not self.authorized_token or get_datetime(doc.authorized_token_expiration_time) <= now_datetime():
                ...
                doc.authorized_token_expiration_time = add_to_date(
                    None, hours=24)
                doc.save()
                frappe.db.commit()
    ...
    def get_delivery_services(self, items):
        ...
        try:
            ...
            return {
                'options': parsed,
                'cannot_add': adding['cannot_add']
            }
        except NoDeliveryOptionsAvailableError:
            frappe.msgprint('Нет доступных способов доставки',
...
def get_sales_orders_containing_items(items_in_options, sales_orders):
    ...
    for option in items_in_options:
        ...
        for item in items_in_options[option]:
            sales_orders_with_item = frappe.db.get_list('Sales Order', filters={
                'name': ['in', sales_orders], 'item_code': ['in', [item]]},
                fields=['name'], as_list=True)
            items_in_sales_orders[item] = [s[0]
                                           for s in sales_orders_with_item]
        items_in_sales_orders_by_options[option] = items_in_sales_orders
    return items_in_sales_orders_by_options


@frappe.whitelist()
def get_unavailable_items_in_cart_by_orders(unavailable_items, sales_orders, items_to_sell):
    ...
    for d in ('Sales Order Item', 'Sales Order Child Item'):
        so_items += frappe.get_all(
            d, ['item_code', 'item_name', 'qty', 'parent'],
            {'parent': ['in', sales_orders],
             'item_code': ['in', list(unavailable_items_map.keys())]},
            order_by='modified desc')

    item_names = frappe.get_all('Item', ['item_code', 'item_name'], {'item_code': [
                                'in', [d['item_code'] for d in items_to_sell]]})
...
def get_sales_order_query(doctype, txt, searchfield, start, page_len, filters):
    ...
    res = frappe.db.sql('''
        ...
        '''.format(scond=searchfields, dont_pass_cond=dont_pass_cond), {
        'dont_pass': dont_pass,
        'txt': "%%%s%%" % txt,
        'start': start,
        'page_len': page_len},
        as_list=True, formatted=True)
...
def get_purchase_info(purchase_id, use_lite_id):
    ...
    purchase_info = None
    try:
        purchase_info = utils.get_purchase_info(
            purchase_id, use_lite_id=use_lite_id)
    except Exception:
        pass
    return {
