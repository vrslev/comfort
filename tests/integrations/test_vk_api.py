...
def test_vk_api_init():
    vk = VkApi()
    assert vk.group_token
    assert type(vk._session) == requests.Session


def test_vk_api_get_group_token_from_settings_with_token(
    vk_api_settings: VkApiSettings,
):
    vk = VkApi.__new__(VkApi)
    vk._get_group_token_from_settings()
    assert vk.group_token == vk_api_settings.group_token


def test_vk_api_get_group_token_from_settings_no_token():
    vk = VkApi.__new__(VkApi)
    with pytest.raises(
...
def test_vk_api_get_params_with_params_plain(vk_api: VkApi):
    assert vk_api._get_params({"key": "value"}) == {
        "access_token": vk_api.group_token,
...
@responses.activate
def test_vk_api_send_message(monkeypatch: pytest.MonkeyPatch, vk_api: VkApi):
    user_id = 1
    message = "test message"
    monkeypatch.setattr(comfort.integrations.vk_api, "randint", lambda *args: 20)  # type: ignore
    params = {"user_id": str(user_id), "message": copy(message), "random_id": "20"}
    add_mock_response(vk_api, "messages.send", params, {"foo": "bar"})
    vk_api.send_message(user_id=user_id, message=message)
